...
The table below lists the MEPs with Z scores greater than `r SCCZscoreThresh` or less than -`r SCCZscoreThresh`.
...
##Spot Position
Plots of the mean local X (Y) coordinate per spot vs array row.
```{r, eval = FALSE}

l1 <- l1[,meanX := mean(Nuclei_CP_AreaShape_Center_X, na.rm = TRUE), by="Barcode,Well,Spot"]

l1 <- l1[,meanY := mean(Nuclei_CP_AreaShape_Center_Y, na.rm = TRUE), by="Barcode,Well,Spot"]

```

```{r SpotPositionplots,echo=FALSE, fig.width=8, fig.height=5, eval=FALSE}
for (barcode in barcodes){
  DT <- l1[l1$Barcode == barcode]
p <- ggplot(DT, aes(x = ArrayRow, y = meanX))+ geom_point()+
  stat_smooth(method = "loess", span = .4)+
  facet_wrap(~Well)+
  ggtitle(barcode)
print(p)
}
...
for (barcode in barcodes){
  DT <- l1[l1$Barcode == barcode]
p <- ggplot(DT, aes(x = ArrayRow, y = meanY))+ geom_point()+
  stat_smooth(method = "loess", span = .4)+
  facet_wrap(~Well)+
  ggtitle(barcode)
print(p)
