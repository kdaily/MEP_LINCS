...
plotCenteredBoxes(dt=l1MEPs, value="mel-m.l", groupBy="ECMp",
...
plotCenteredBoxes(dt=l1MEPs, value=value, groupBy=groupBy, titleExpression=titleExpression)

plotCenteredBoxes(dt=l1MEPs, value="mel-me.", groupBy="Barcode", colourBy="Well", titleExpression=expression(EdU^{"+"}~Centered~Medians~by~Ligand~(m["e,l"]*"-"*m["e,."])))
...
plotCenteredBoxes(l1MEPs, value="Mel-M.l", groupBy="ECMp",
                  titleExpression=expression(EdU^{"+"}~Centered~MADs~by~ECMp~(M["e,l"]*"-"*M[".,l"])))

plotCenteredBoxes(l1MEPs, value="Mel-Me.", groupBy="Ligand",
                  titleExpression=expression(EdU^{"+"}~Centered~MADs~by~Ligand~(M["e,l"]*"-"*M["e,."])))

plotCenteredBoxes(l1MEPs, value="Mel-Me.", groupBy="Barcode", colour="Well",
                  titleExpression=expression(EdU^{"+"}~Centered~MADs~by~Ligand~(M["e,l"]*"-"*M["e,."])))

```


```{r, eval=TRUE, fig.width=9, fig.height=6, eval=FALSE}
k <- nrow(YEdU)
cIdx <- 1:ncol(YEdU)

for(nu in c(1e-3, 1e-2, 1e-1, 1)){
  nYEdUm <- medianNaiveRUV2(nu, YEdU, cIdx, k)
  setnames(nYEdUm, "Value", "mel")
  
  nYEdUM <- madNaiveRUV2(nu, YEdU, cIdx, k)
  setnames(nYEdUM, "Value", "Mel")
  setkey(nYEdUm,MEP,ECMp,Ligand)
  setkey(nYEdUM, MEP,ECMp,Ligand)
  ...
  plotLEHmap(dt=nYEdU, fill="mel",
             titleExpression=bquote(EdU^{"+"}~ Medians~(m["e,l"])~RUV~with~nu~"=" ~.(nu)*","~k~"="~.(k)))
  
  plotLEHmap(dt=nYEdU, fill="Mel",
             titleExpression=bquote(EdU^{"+"}~ MADs~(m["e,l"])~RUV~with~nu~"=" ~.(nu)*","~k~"="~.(k)))
  
  plotCenteredBoxes(dt=nYEdU, value="mel-m.l", groupBy="ECMp",
                    titleExpression=expression(EdU^{"+"}~Centered~Medians~by~ECMp~(m["e,l"]*"-"*m[".,l"])))
  
  plotCenteredBoxes(dt=nYEdU, value="mel-me.", groupBy="Ligand",
                    titleExpression=expression(EdU^{"+"}~Centered~Medians~by~Ligand~(m["e,l"]*"-"*m["e,."])))
  ...
  plotCenteredBoxes(dt=nYEdU, value="Mel-Me.", groupBy="Ligand",
                    titleExpression=expression(EdU^{"+"}~Centered~MADs~by~Ligand~(M["e,l"]*"-"*M["e,."])))
  ##Todo: bring Barcode and well along with MEP, etc.
  # plotCenteredBoxes(dt=nYEdU, value="mel-me.", groupBy="Barcode", colourBy="Well", titleExpression=expression(EdU^{"+"}~Centered~Medians~by~Ligand~(m["e,l"]*"-"*m["e,."])))
}
```

```{r}

nYEdUList <- lapply(c(5,10,20, 40), function(k, YEdU, cIdx, mValue, MValue){
...
p <- ggplot(dt,aes_string(x=groupBy, y=value, colour=colourBy))+
  ...
  guides(fill=FALSE)+
  theme_grey(base_size = 12, base_family = "")+
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1, size=rel(.6)), axis.text.y = element_text(angle = 0, vjust = 0.5, hjust=1, size=rel(1)), plot.title = element_text(size = rel(1)),legend.text=element_text(size = rel(.3)),legend.title=element_text(size = rel(1)))+
